(function () {
  ...
  // Capture script URL once; document.currentScript may be null on SPA navigations
  const thisScript =
    document.currentScript ||
    document.querySelector('script[src$="glossary.js"]');
  const baseUrl = (thisScript && thisScript.src) || location.href;
  ...
  function walk(node, re, termsMap) {
    const walker = document.createTreeWalker(node, NodeFilter.SHOW_TEXT, {
      acceptNode(n) {
        ...
        if (
          p.closest(
            "a, code, pre, kbd, samp, .glossary-term, .no-glossary, .md-header, .md-nav"
          )
        )
  ...
  function enhance() {
    fetch(new URL("glossary.json", baseUrl))
      .then((r) => r.json())
      .then((data) => {
        ...
        const map = {};
        terms.forEach((t) => (map[t.toLowerCase()] = data[t]));
